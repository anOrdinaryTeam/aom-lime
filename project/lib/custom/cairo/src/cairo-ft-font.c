...
/*
 * FreeType < 2.10 has no COLR/CPAL palette API (ftcolor.h). Unlike SDF above,
 * cairo uses it unconditionally, so building against old FreeType fails. Provide
 * no-op shims of the exact API this file uses. Effect: COLR *vector* color fonts
 * are unsupported; color *bitmap* glyphs (CBDT/sbix via FT_LOAD_COLOR) are
 * unaffected. This whole block disappears automatically once FreeType is >= 2.10.
 *
 * lime custom: keep this file in sync with lib/cairo/src/cairo-ft-font.c on bumps.
 */
...
static FT_Error
FT_Palette_Data_Get (FT_Face face, FT_Palette_Data *adata)
{
    (void) face;
    if (adata)
        adata->num_palettes = 0;
    return 1; /* nonzero: unavailable */
}
                          ...
                          FT_LayerIterator  *iterator)
{
    ...
    (void) acolor_index;
    (void) iterator;
    return 0; /* FALSE: no color layers */
}
